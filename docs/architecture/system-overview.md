...
The frontend also carries a checked-in manager reference database built from the supplied `sm-data` and `sm-actives` snapshots. It is presentation and partial-package fallback data, joined by stable manager identity; it does not replace or mutate the immutable release package.
